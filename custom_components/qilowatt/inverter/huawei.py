...
class HuaweiInverter(BaseInverter):
    ...
    def get_metrics_data(self):
        ...
        # Load Power calculation with ML adjustment for phase balancing
        house_load_median = self.get_state_float("house_load_median")
        battery_power_value = self.get_state_float("batteries_charge_discharge_power")

        # Start with the actual house load
        load_power_total = house_load_median

        # Check if battery is discharging (negative value = discharging)
        if battery_power_value is not None and battery_power_value < 0 and load_power_total is not None:
            # Battery is discharging - check if we're maintaining positive phases
            
            # Get individual phase powers (positive = export, negative = import)
            power_a = self.get_state_float("power_meter_phase_a_active_power")
            power_b = self.get_state_float("power_meter_phase_b_active_power")
            power_c = self.get_state_float("power_meter_phase_c_active_power")
            
            # Sum up all positive (exporting) phases
            # These represent extra energy the battery had to provide beyond house load
            export_adjustment = 0
            if power_a is not None and power_a > 0:
                export_adjustment += power_a
            if power_b is not None and power_b > 0:
                export_adjustment += power_b
            if power_c is not None and power_c > 0:
                export_adjustment += power_c
            
            # Add the export values to show the ML algorithm the true demand
            # This represents: house load + energy needed to maintain positive phases
            if house_load_median is not None:
                load_power_total = house_load_median + export_adjustment
